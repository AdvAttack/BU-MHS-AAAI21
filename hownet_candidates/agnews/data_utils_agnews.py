...
class AGNewsDataset(object):
    def __init__(self, path='ag_news_csv', max_vocab_size=None):
        self.path = path
        ...
        # self._read_vocab()
        self.train_text, self.train_y, self.test_text, self.test_y = self.split_agnews_files()
        print('tokenizing...')
    ...
    def read_agnews_files(self, filetype):
        texts = []
        ...
        path = r'./ag_news_csv/{}.csv'.format(filetype)
        csvfile = open(path, 'r')
    ...
    def split_agnews_files(self):
        print("Processing AG's News dataset")
        train_texts, train_labels, _ = self.read_agnews_files('train')  # 120000
        test_texts, test_labels, _ = self.read_agnews_files('test')  # 7600
