...
def read_yahoo_csv_files(filetype):
    texts = []
    labels_index = []  # The index of label of all input sentences, which takes the values 1,2,3,4
    doc_count = 0  # number of input sentences
    path = r'./data_set/yahoo_answers_csv/{}.csv'.format(filetype)
    csvfile = open(path, 'r')
    for line in csv.reader(csvfile, delimiter=',', quotechar='"'):
        content = line[1] + ". " + line[2]
        texts.append(content)
    ...
    labels = []
    for i in range(doc_count):
        label_class = np.zeros(config.num_classes['yahoo_csv'], dtype='float32')
        label_class[int(labels_index[i]) - 1] = 1
        labels.append(label_class)
...
def split_yahoo_csv_files():
    print("Processing Yahoo CSV dataset")
    train_texts, train_labels, _ = read_yahoo_csv_files('train')  # 1400000
    test_texts, test_labels, _ = read_yahoo_csv_files('test')  # 60000
    return train_texts, train_labels, test_texts, test_labels
